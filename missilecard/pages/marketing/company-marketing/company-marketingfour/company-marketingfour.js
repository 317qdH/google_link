// pages/marketing/company-marketing/company-marketingfour/company-marketingfour.js
var app = getApp();
...
Page({
  ...
  async saveToImage(e) {
    ...
    //完成名片canvas制作
    // const { rpx } = await toCardPoster.enterpriseMarketing2(this, ctx, this.data.cardImageSrc);
    await toCardPoster.enterpriseMarketing4(this, ctx, userInfoObj, introduction, this.data.qrcodeUrl);
  ...
  async generateQRcode(userInfoObj) {
    let response = await api.mini_qr({
      query: {
        ...
        page: 'pages/marketing/company-marketing/company-marketingfour/company-marketingfour'
      },
      method: 'get',
      header: {
  ...
  async markCominfo() {
    ...
    if (res.data.success) {
      ...
      wx.setStorageSync('company4', data)
    } else {
      wx.showToast({
        icon: 'none',
        title: '网络异常',
  ...
  onShareAppMessage: function (res) {
    if (res.from === 'button') {
      ...
      return {
        ...
        path: '/pages/marketing/company-marketing/company-marketingfour/company-marketingfour?id=' + this.data.twoinfo.id + '&userId=' + this.data.twoinfo.userId,
