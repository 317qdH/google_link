// pages/marketing/company-marketing/company-marketingthr/company-marketingthr.js
var app = getApp();
...
Page({
  ...
  async saveToImage(e) {
    ...
    //完成名片canvas制作
    await toCardPoster.enterpriseMarketing3(this, ctx, userInfoObj, introduction, this.data.qrcodeUrl);
    // ctx.draw();
    //canvas导出为图片
    ctx.draw(true, ()=>{
      ...
      wx.login({
        success(res) {
          if (res.code) {
            ...
            wx.request({
              method: 'post',
              url: api.BASE_PATH + '/carte/authorization_by_code', 
              header: {
  ...
  async generateQRcode(userInfoObj) {
    let response = await api.mini_qr({
      query: {
        scene: this.data.thrinfo.id + '-' + this.data.thrinfo.userId,
        page: 'pages/marketing/company-marketing/company-marketingthr/company-marketingthr'
      },
      method: 'get',
      header: {
  ...
  async markCominfo() {
    ...
    if (res.data.success) {
      ...
      wx.setStorageSync('company3', data)
    } else {
      wx.showToast({
        icon: 'none',
        title: '网络异常',
  ...
  onShareAppMessage: function (res) {
    if (res.from === 'button') {
      // 来自页面内转发按钮
      return {
        title: '您好！我们是' + this.data.thrinfo.companyName + '，期待与您的合作！',
        path: '/pages/marketing/company-marketing/company-marketingthr/company-marketingthr?id=' + this.data.thrinfo.id + '&userId=' + this.data.thrinfo.userId,
